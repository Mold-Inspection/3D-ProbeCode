...
class CustomizationTab:
    ...
    def draw_cross_section(self):
        ...
        if has_hole:
            ...
            ax3d.scatter(tx_list[:wall_n], ty_list[:wall_n], tz_list[:wall_n], color='red', s=22, depthshade=False, label=f'Wall Contact ({layers}L×{points}P)')
            ax3d.scatter([tx_list[-1]], [ty_list[-1]], [tz_list[-1]], color='#ffea00', s=90, marker='*', depthshade=False, label='Bottom Depth Point', zorder=10)
            ...
            title_str = (f"Customization — Hole {hole.id}  |  R={hole.radius:.1f} mm  Depth={hole.depth:.2f} mm  |  {layers}L × {points}P = {layers*points} pts" + (' [STEP]' if has_step_hole else ' [Mesh]'))
