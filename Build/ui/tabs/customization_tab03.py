...
class CustomizationTab:
    ...
    def draw_cross_section(self):
        ...
        if has_hole:
            ...
            if wall_pts:
                if use_zigzag:
                    # สร้าง colormap ตาม layer_idx
                    max_lidx = max(wp[3] for wp in wall_pts) or 1
                    cmap_z   = plt_colormap(layers)
                    for lidx in range(layers):
                        ...
                        color_l = cmap_z(lidx / max_lidx)
                        lbl = f'Layer {lidx+1} (+{int(round(lidx * step_deg))}°)' if lidx > 0 else f'Layer 1 (0°)'
                        ax3d.scatter(wx_l, wy_l, wz_l,
                                     color=color_l, s=22, depthshade=False, label=lbl)
                else:
...
def plt_colormap(n: int):
    """คืน callable colormap สำหรับแบ่งสี n layers แบบ rainbow"""
