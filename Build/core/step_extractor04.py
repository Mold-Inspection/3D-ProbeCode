...
#               separate, axially-stacked holes into one. At that point
#               _MAX_RADIUS_RATIO was left at 2.0 as a placeholder, which
#               was NOT tight enough — the field data showed
#               stacked-but-separate holes touching end-to-end (gap ~= 0)
#               with radius ratios of 1.4x-1.56x, so gap alone could not
#               distinguish them from real counterbores.
#   v04 -> v05: REAL FIX (confirmed against field data) — tightened
#               _MAX_RADIUS_RATIO from 2.0 to 1.2. Verified against every
#               false-merge pair observed in the user's STEP file log:
#                 (133,83)  r=15 vs r=21  ratio=1.40  -> now KEPT SEPARATE
#                 (0,95)    r=8  vs r=12.5 ratio=1.56  -> now KEPT SEPARATE
#                 (-80,90)  r=10 vs r=15  ratio=1.50  -> now KEPT SEPARATE
#                 (-133,83) r=15 vs r=21  ratio=1.40  -> now KEPT SEPARATE
#                 (80,90)   r=10 vs r=15  ratio=1.50  -> now KEPT SEPARATE
#               All 5 confirmed false merges from the user's part are
#               rejected at this threshold. A genuine tight counterbore
#               (e.g. spot-face/chamfer step under 1.2x) will still merge
#               correctly. User confirmed via explicit choice: "Keep
#               merging, but only when radii are very close (ratio < 1.2x)".
#   v05 -> v06: ADDED FILE LOGGING (no geometry/merge logic changed).
#               Every _dbg() call now writes to BOTH the console (unchanged
#               behavior) AND a timestamped log file under Build/Log/, so
#               full run output can be reviewed later without depending on
#               copy-pasting from a terminal.
#                 - Log folder: <Build>/Log/  (created automatically if
#                   missing; <Build> is resolved relative to this file's
#                   location, i.e. core/.. , so it works regardless of the
#                   process's current working directory).
#                 - Log filename: Log_<YYYY-MM-DD_HH-MM-SS>.txt, timestamped
#                   at first write of each run (module import time when
#                   DEBUG is True), so every run gets its own file.
#                 - The log file handle is opened lazily on first _dbg()
#                   call and kept open for the lifetime of the process.
#                 - If the Log folder or file cannot be created (e.g.
#                   permissions, read-only filesystem), file logging is
#                   silently disabled and console logging continues
#                   unaffected — this must never crash the application.
#
